...
- **自律git操作の安全装置をF-05に追加（M8実装時に必須）**：Hiveは `hive/` プレフィックスのブランチのみpush可・PRはドラフト作成・mainへの直接push禁止。取り消しコストが高い操作は構造的に不可能にする
...
- **無人実行の安全装置（M8実装時に必須）**
  ...
  - PRは必ず**ドラフト**として作成し、マージ判断は常に人間に残す
...
| 自律git操作の安全装置 | `hive/`ブランチのみpush可・ドラフトPR・main直push禁止（F-05） | 取り消しコストが高い操作は権限レベルで構造的に不可能にする。無人実行の前提条件 |
