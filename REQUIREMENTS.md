...
*作成日：2026年5月*
*更新日：2026年6月10日（v2.4〜v2.6・§0参照）：ADK 2.2.0対応（v2.4）＋品質要件の強化（受け入れ基準の上流定義・検証役の責任分離・レビュー出力フォーマット）（v2.5）＋自動化・無人実行の要件追加（git安全装置・トリガー発注・コスト暴走対策）（v2.6）。※冒頭のvX.X（要件改訂）と以下の更新履歴のvX.X（旧採番）は系列が異なる点に注意*
*更新日：2026年5月30日（v2.7）*
