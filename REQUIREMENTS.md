...
- **fullstack成果物の受け取り体験を出荷基準に合わせる（F-04）**：「`uvicorn main:app --port 8001` で起動してください」は非エンジニアの語彙に存在せず、成果物が実質「動かせない」状態だった。①成果物カードで「これはAPI起動が必要な開発者向けです／ブラウザだけで動くものが欲しいなら『TODOアプリを作って』と発注してください」と壁の存在を先に開示しapp動線へ誘導 ②`python main.py` だけで起動する形をimplementerに義務づけ（`shared/sandbox.py: check_selfstart` で機械チェック＝満たさなければF-04ループで差し戻し）、ダブルクリック起動の `start.bat` / `start.command` を同梱。※「PythonがPCに入っている」前提だけは越えられないので画面に明示する（この壁を無くす本命は成果物のURL返し＝将来のP2）
...
- **機械検証の同時実行の上限（F-03 安定化）**：ブラウザ実行・uvサンドボックスは検証1件ごとに重いサブプロセスを立てるため、多人数が同時発注するとCPUを取り合いタイムアウトが多発する。Semaphoreで同時に走る検証を絞り（`HIVE_MAX_CONCURRENT_VERIFY` 既定3・"0"以下で無効）、超過分は順番待ちにする（待つだけ・失敗にはしない）。生成アプリの利用側（app＝各自ブラウザ／fullstack＝各自PC）は元々Hiveのサーバー負荷ゼロなので対象外＝これは「発注の同時実行」を守る防御。本番はCloud Runの水平スケールと併用
...
- **安定化＝「固まらない」保証（v2.11実装済み）**
  ...
  - 機械検証の同時実行の上限：ブラウザ実行・uvサンドボックスは1件ごとに重いサブ
    プロセスを立てるため、多人数の同時発注でCPUを取り合いタイムアウトが多発する。
    Semaphoreで絞り（`HIVE_MAX_CONCURRENT_VERIFY` 既定3・"0"以下で無効・`_verify_artifact`）、
    超過分は順番待ちにする（待つだけ・失敗にはしない）。生成アプリの利用側（app＝各自
    ブラウザ／fullstack＝各自PC）はHiveのサーバー負荷ゼロで対象外。本番はCloud Run併用
...
- **fullstack成果物の出荷基準（v2.11）＝「受け取った人が起動できる」**
  - appは「開くだけで動く」が担保されるが、fullstackはAPI起動という壁がある。
    壁を消せない以上、**壁の存在を先に開示し・越え方を最短にする**のを基準とする：
    1. **壁の開示とapp動線への誘導**：成果物カードで「これはAPIサーバーの起動が必要な
       開発者向け」と明示し、「ブラウザだけで動くものが欲しいなら『TODOアプリを作って』
       と発注」と案内する（プレビューのエラー表示が故障ではないことも伝える）
    2. **`python main.py` だけで起動できる**：implementerに `if __name__ == "__main__":`
       での `uvicorn.run(app, port=8001)` を義務づけ、`shared/sandbox.py: check_selfstart`
       が機械チェック（不合格ならF-04ループで差し戻し）。import時に実行されないため
       サンドボックスのpytestには影響しない
    3. **コマンドを打たせない**：`start.bat`（Windows）/ `start.command`（Mac）を同梱し、
       ダブルクリック→`index.html` を開く、の2手で動く
  - 残る前提「PythonがPCに入っていること」は越えられないので画面に明示する。
    この壁自体を無くす本命は**成果物のURL返し**（生成APIをCloud Runへ自動デプロイし
    「発注→動くURLが届く」）＝将来のP2。セキュリティ・課金・工数が重いため別マイルストーン
